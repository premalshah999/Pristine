...
import { getPublicServiceDetails, services } from "@/lib/services";
        ...
        <div className="grid grid-cols-12 gap-y-8 gap-x-0 md:gap-x-10 mb-12">
          ...
          <Reveal className="col-span-12 md:col-span-7 md:pt-2" delay={100}>
            <p className="lede max-w-[58ch]">
              The work is connected. Assessment may inform the map, nutrition
              and lifestyle may change the terrain, and coaching keeps the plan
              usable when that level of support fits.
            </p>
            <p className="mt-5 max-w-[58ch] text-[0.86rem] leading-relaxed text-[var(--color-ink-muted)]">
              These are broad examples. Specific tests, supplements, and
              protocol details are discussed privately only when they are
            ...
            {services.map((service) => {
              ...
              >
                ...
                <span className="grid grid-cols-2 xl:grid-cols-3 border-r px-5 py-7" style={{ borderColor: "var(--color-line-strong)" }}>
                  {publicDetails.focusAreas.slice(0, 3).map((item) => (
                    <span
                      key={item.title}
            ...
            <Reveal key={service.slug} delay={index * 50}>
              <Link href={`/services/${service.slug}`} className="block py-6">
                ...
                <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {publicDetails.focusAreas.slice(0, 4).map((item) => (
                    <span
                      key={item.title}
